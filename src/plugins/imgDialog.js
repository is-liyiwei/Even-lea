...
export default {
  install(Vue) {
    ...
    window.openImgDialog = function(targetDom, srcKey) {
      ...
      let imgPreviewBoxDomRect = imgPreviewBox.getBoundingClientRect();
      let w_diff = targetDomRect.width - imgPreviewBoxDomRect.width;
      let imageDom = imgPreviewBox.querySelector('img');
      imageDom.src = window.qCode[srcKey];
      imgPreviewBox.style.top =
        targetDomRect.y - imgPreviewBoxDomRect.height - 20 + 'px';
      imgPreviewBox.style.left = targetDomRect.x + w_diff / 2 + 'px';
      setTimeout(() => {
